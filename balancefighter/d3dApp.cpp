...
D3DApp::D3DApp(HINSTANCE hInstance)
{
	...
	mClientWidth    = WINDOW_WIDTH;
	mClientHeight   = WINDOW_HEIGHT;
}
...
int D3DApp::run()
{
	...
	while(msg.message != WM_QUIT)
	{
        ...
        {	
			mTimer.tick();
			FrameTimer.tick();
			float FrameTime = FrameTimer.getDeltaTime();

			if( !mAppPaused )
			{
				/* Guard so we only draw at 60fps*/
				//while(FrameTime <= 0.01666666667)
				{
					updateScene(mTimer.getDeltaTime());	
					FrameTimer.tick();
					FrameTime += FrameTimer.getDeltaTime();
				}
			}
